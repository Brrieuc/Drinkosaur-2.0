...
export const FriendProfileModal: React.FC<FriendProfileModalProps> = ({
    ...
    useEffect(() => {
        if (onUpdateFriend && isFriend) {
            onUpdateFriend(fullStatus);
        }
    }, [fullStatus, onUpdateFriend, isFriend]);
    ...
    const t = {
        ...
        at: isFrench ? 'à' : '@',
        privateProfile: isFrench ? 'Profil Privé' : 'Private Profile',
        addFriend: isFrench ? 'Ajouter en ami' : 'Add Friend',
        requestsDisabled: isFrench ? 'Demandes désactivées' : 'Requests Disabled'
    };
    ...
    const { gradientStyle, glowStyle } = useMemo(() => {
        ...
        if (!isFriend || fullStatus.currentBac <= 0 || friendDrinks.length === 0) {
            return {
    ...
    }, [friendDrinks, fullStatus.currentBac, isFriend]);
