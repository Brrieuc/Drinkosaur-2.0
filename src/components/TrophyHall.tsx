...
export const TrophyHall: React.FC<TrophyHallProps> = ({
    ...
    return createPortal(
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/90 backdrop-blur-3xl animate-fade-in pointer-events-auto">
            <div className="w-full max-w-lg bg-[#0a0a0a] rounded-[40px] border border-white/10 shadow-2xl relative animate-scale-up max-h-[90vh] flex flex-col modal-container pointer-events-auto overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between p-6 bg-white/[0.02] border-b border-white/5">
                    <button
                        onClick={onClose}
                        className="p-2 bg-white/5 rounded-full text-white/40 hover:bg-white/10 transition-colors"
                    >
                        <X size={24} />
                    </button>
